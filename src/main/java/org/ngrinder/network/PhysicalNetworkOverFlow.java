...
import org.ngrinder.infra.ContextContainer;
 ...
 * @since 3.3
 */
public class PhysicalNetworkOverFlow extends SpringPlugin {
	...
	public static class PhysicalNetworkOverFlowExtension implements
		...
		public void checkWorkingAgent(Set<AgentStatus> workingAgents) {
			...
			if (totalReceived > limit || totalSent > limit) {
				LOGGER.debug("LIMIT : {}, RX : {}, TX : {}", new Object[]{limit, totalReceived,
					totalSent});
				for (PerfTest perfTest : getAllTesting()) {
		...
		List<PerfTest> getAllTesting() {
			return ((IPerfTestService) ContextContainer.getContainer(IPerfTestService.class)).getAllTesting();
		}

		long getLimit() {
			return config.getControllerProperties().getPropertyInt(PROP_NETWORK_OVERFLOW_LIMIT,
